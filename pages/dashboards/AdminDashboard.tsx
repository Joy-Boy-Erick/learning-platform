...
const AdminDashboard: React.FC = () => {
  ...
  const createUserModalRef = useRef<HTMLDivElement>(null);
  const createUserTriggerRef = useRef<HTMLElement | null>(null);
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      if (event.key === 'Escape' && !isCreatingUser) setIsModalOpen(false);
      if (event.key === 'Tab') {
        const focusableElements = createUserModalRef.current?.querySelectorAll<HTMLElement>(
    ...
    if (isModalOpen) {
      createUserTriggerRef.current = document.activeElement as HTMLElement;
      document.addEventListener('keydown', handleKeyDown);
      setTimeout(() => {
        const firstInput = createUserModalRef.current?.querySelector('input');
        firstInput?.focus();
      }, 100);
    } else if (createUserTriggerRef.current) {
        createUserTriggerRef.current.focus();
    }
    
    return () => {
      document.removeEventListener('keydown', handleKeyDown);
      if (!isModalOpen && createUserTriggerRef.current) {
        createUserTriggerRef.current.focus();
      }
    };
  ...
  const StarRatingDisplay = ({ rating }: { rating: number }) => (
    <div className="flex items-center" role="img" aria-label={`${rating.toFixed(1)} out of 5 stars`}>
        {[...Array(5)].map((_, i) => (
        <svg key={i} aria-hidden="true" className={`w-4 h-4 ${i < rating ? 'text-yellow-400' : 'text-gray-300 dark:text-gray-600'}`} fill="currentColor" viewBox="0 0 20 20">
      ...
      <div id="tabpanel-reviews" role="tabpanel" tabIndex={0} aria-labelledby="tab-reviews" hidden={activeTab !== 'reviews'}>
       ...
       {isModalOpen && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-labelledby="create-user-title">
          <div ref={createUserModalRef} className="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6 w-full max-w-md" role="document">
            <div className="flex justify-between items-center mb-4">
              <h2 id="create-user-title" className="text-lg font-bold text-dark dark:text-light">Create New User</h2>
              <button onClick={() => setIsModalOpen(false)} aria-label="Close create user dialog" className="p-1 rounded-full hover:bg-gray-200 dark:hover:bg-gray-700">&times;</button>
            </div>
